...
describe('maintained repository reference policy', () => {
  ...
  it('excludes the fork Desktop governance that records its synchronized upstream target', () => {
    const commit = 'a'.repeat(40)
    const commits = new Set([commit])
    for (const file of [
      'AGENTS.md',
      'desktop/UPSTREAM_COMMIT',
      '.agents/notes/implemented/process/2026-09-14-desktop-sync-dsh-0.1.5-rc.2.md',
      '.agents/notes/implemented/process/2026-08-22-desktop-upstream-ancestry.zh.md',
    ]) {
      expect(findRepositoryReferences(file, commit, commits)).toEqual([])
    }
    expect(findRepositoryReferences('.agents/notes/implemented/process/2026-09-14-other.md', commit, commits))
      .toEqual([{ file: '.agents/notes/implemented/process/2026-09-14-other.md', line: 1, kind: 'commit-hash' }])
  })
})
