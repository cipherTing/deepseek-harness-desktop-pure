...
/**
 * DeepDive's own Desktop governance names the exact upstream commit every
 * synchronization covers, which its policy requires by hash as well as by tag.
 */
const desktopGovernance = new RegExp([
  '^AGENTS\\.md$',
  '^desktop/UPSTREAM_COMMIT$',
  '^\\.agents/notes/implemented/process/\\d{4}-\\d{2}-\\d{2}-desktop-(?:sync|upstream-ancestry)',
].join('|'))
...
function isMaintained(file: string): boolean {
  if (desktopGovernance.test(file)) return false
 ...
 * @returns Prohibited references outside vendor, frozen Agent Notes, and the fork's own
 * Desktop governance; absent shallow-history objects cannot match.
 */
