...
弹窗报告准备中、浏览器开始下载、原生文件保存完成或失败。关闭弹窗不会取消正在进行的下载；该操作随后完成时也不会重新打开弹窗。每个 Session 同时只允许一项下载，重复操作会共用该任务。客户端启动前可以安装异步保存载体；默认浏览器下载行为保持不变。
...
- 默认行为是浏览器下载，不是 Host 路径写入。Desktop 可以安装原生保存载体来选择文件目标，并报告取消或完成，但不会向页面暴露通用文件系统 API。
