...
DeepDive 是本 fork 独立的 Tauri 桌面发行版。它把标准 `web` profile 与随包 Node.js sidecar 一起打包，在 Tauri WebView 中通过 loopback 地址加载，同时与原生 Web 共享 Harness 配置和用户数据。Desktop 的生命周期、打包和原生集成规则定义在仓库的 [AGENTS.md](../AGENTS.md) 中。
