...
Web and Desktop ship their own command pair. The Desktop commands build and launch the DeepDive Desktop application; `start:web` launches the artifacts of a prior `pnpm run build`, while `dev:web` runs that build first and then launches. Web additionally keeps client bundles rebuilt on source edits, because its Host runs from source while the browser loads built bundles:
...
pnpm run desktop:build   # build the DeepDive Desktop application
pnpm run desktop:dev     # build, then launch the DeepDive Desktop application
```

Arguments after a Web command reach `dsh web`, for example `pnpm run dev:web --no-open --port 3081`; `dev:web` also accepts `--skip-build` to reuse the existing artifact tree and `--no-serve` to run only the rebuild watchers beside a server started elsewhere. Both Web commands use the normal Harness home, and the Desktop commands share it. The root `Makefile` names the same commands as `make web`, `make dev-web`, `make desktop`, `make dev-desktop`, and `make build`; `ARGS='--no-open'` forwards options.
