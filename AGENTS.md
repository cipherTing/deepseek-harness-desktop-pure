...
Never reset or rebuild from upstream, use a path allowlist, create an upstream-rooted sync branch, or use `-s ours` to bypass review. The only exception is the named tree-preserving `dsh-v0.1.1-rc.2` ancestry repair at `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e` ([decision](.agents/notes/implemented/process/2026-08-22-desktop-upstream-ancestry.md)); never reuse it.

The sync merge uses `master` as first parent and the verified upstream target as second parent. The landing merge uses current `master` as first parent and the reviewed sync branch as second parent, both with `--no-ff`. Never reset or rebase published `master` or release tags.

Before completion, verify the exact target identity, ancestry, zero target-only commits, both merge parent orders, and equal sync/landing trees. Report upstream counts with `git rev-list --left-right --count <verified-upstream-sha>...HEAD`; `origin/master...HEAD` measures delivery only. `desktop/UPSTREAM_COMMIT` contains the exact tag or full SHA.
